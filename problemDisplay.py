...
class ProblemDisplay:
    ...
    def draw(self):
        if (self.stateManager.getCurrentState() != "Finished"): # only draw two problem fractions and x symbol
            ...
            if self.stateManager.operation_type == 2 or self.stateManager.operation_type == 4 or self.stateManager.operation_type == 3 or self.stateManager.operation_type == 1: # division or addition
                self.userdenominator = self.stateManager.get_answerDenom()
